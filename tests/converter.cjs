...
async function main() {
  ...
  try {
    ...
    await page.locator("#accountPassword").fill("demo123");
    await page.locator("#accountCreateForm").evaluate((form) => form.requestSubmit());
    await page.locator('[data-account-row]').waitFor({ state: "visible" });
    await page.screenshot({ path: "/tmp/pm-workbench-account-settings.png", fullPage: true });
    const storedAccounts = await page.evaluate(() => localStorage.getItem("orbit-managed-accounts"));
    assert.ok(storedAccounts.includes("demo.user"));
    assert.equal(storedAccounts.includes("demo123"), false);
    await page.locator("#logoutButton").click();
    await page.locator("#loginUsername").fill("demo.user");
    await page.locator("#loginPassword").fill("demo123");
    await page.locator("#loginForm").evaluate((form) => form.requestSubmit());
    await page.locator("#appShell").waitFor({ state: "visible" });
    assert.equal(await page.locator("#adminSettingsNav").isVisible(), false);
    assert.equal(await page.locator("#profileName").textContent(), "demo.user");
    await page.locator("#logoutButton").click();
    await page.locator("#loginUsername").fill("admin");
    await page.locator("#loginPassword").fill("admin");
    await page.locator("#loginForm").evaluate((form) => form.requestSubmit());
    ...
    await page.locator("#gifCoverDownload").click();
    const coverDownload = await coverDownloadEvent;
    assert.equal(coverDownload.suggestedFilename(), "animation-cover-72x72.png");
    ...
    const coverPixels = await page.evaluate(async (base64) => {
      ...
      return {
        firstFrame: Array.from(context.getImageData(9, 18, 1, 1).data),
        transparentCorner: Array.from(context.getImageData(71, 71, 1, 1).data),
      };
    }, coverBytes.toString("base64"));
    assert.deepEqual(coverPixels.firstFrame.slice(0, 3), [255, 0, 0]);
    assert.ok(coverPixels.firstFrame[3] > 200);
    assert.equal(coverPixels.transparentCorner[3], 0);
    ...
    assert.deepEqual({ ...emoji, timeline: undefined }, { version: "2.0", FPS: 20, frames: 420, videoSize: { width: 240, height: 240 }, timeline: undefined });
    const emojiCycle = [ ...Array(2).fill(["frame_0"]), ...Array(4).fill(["frame_1"]), ...Array(6).fill(["frame_2"]), ...Array(2).fill(["frame_3"]) ];
    assert.deepEqual(emoji.timeline, Array.from({ length: 30 }, () => emojiCycle).flat());
